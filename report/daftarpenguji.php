<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Daftar Pengusi</title>
</head>
<body>
    

<style>
    .cop {
        justify-items: center;
        line-height: 8px;
    }

    table {
        border-collapse: collapse;
        width: 100%;
        /* height: 10px; */
        /* height:10px; */
        height: min-content;
        line-height: 8px;
        font-size: 8px;
    }

    table,
    th,
    td {
        border: 1px solid black;
        
    }

    th,
    td {
        padding: 10px;
    }

    th {
        /* background-color: rgb(19, 110, 170); */
        background-color: rgb(90, 150, 170);
        color: white;
    }

    tr:hover {
        background-color: #f5f5f5;
    }
</style>
<table style=" border:0px">
    <tr>
        <td style="border: 0px;width:2px;"><img src="../assets/images/palu.png" alt="" width="50px" height="50px">
        </td>
        <td class="cop" style="border: 0px; text-align:center;width:900px;">
             <h3>PEMERINTAH KOTA PALU</h3>
                <h3>DAFTAR PENGUJI</h3>
                <h5>Nomor : Tanggal :</h5>
        </td>
       
       
    </tr>

</table>

<table style="border: 0px">
    <tr>
        <td style="border: 0px; width:150px">BANK</td>
        <td style="border: 0px; width:2px">:</td>
        <td style="border: 0px">MANDIRI</td>
    </tr>
    <tr>
        <td style="border: 0px; width:150px">NO. REKENING</td>
        <td style="border: 0px; width:2px">:</td>
        <td style="border: 0px">151-000-000-009-8</td>
    </tr>
</table>
<div class="table-responsive">
    <table class="table text-nowrap table-bordered border-primary">
        <tr>
            <th rowspan="2">No</th>
            <th rowspan="2">Tanggal</th>
            <th rowspan="2">Nomor SP2D</th>
            <th rowspan="2">Bruto</th>
            <th colspan="3">Potongan</th>
            <th rowspan="2">Netto</th>
            <th rowspan="2">OPD</th>
            <th rowspan="2">Nomor Rekening</th>
        </tr>
        <tr>
            <th>PPN</th>
            <th>PPh</th>
            <th>Lainnya</th>
        </tr>
        <tr style="height:-50px;margin:0%">
            <td>1</td>
            <td>30/12/2004</td>
            <td>72.71/04.00/01125/LS/0.0.0326.06.0/</td>
            <td>2.000.000</td>
            <td>2.000.000</td>
            <td>2.000.000</td>
            <td>2.000.000</td>
            <td>2.000.000</td>
            <td>Badan Keuangan Daerah dan Aset Daerah</td>
            <td>151-000-000-009-8 </td>
        </tr>
        <tr>
            <td>1</td>
            <td>30/12/2004</td>
            <td>72.71/04.00/01125/LS/0.0.0326.06.0/</td>
            <td>2.000.000</td>
            <td>2.000.000</td>
            <td>2.000.000</td>
            <td>2.000.000</td>
            <td>2.000.000</td>
            <td>Badan Keuangan Daerah dan Aset Daerah</td>
            <td>151-000-000-009-8 </td>
        </tr>
    </table>
</div>
<br>
<div class="container-fluid p-2" >
    <div class="row">
        <table style="border: 0px;line-height: 8px;">
            <tr>
                <td style="border: 0px; width:500px">Total SP2D S/D DAFTAR PENGUJI YANG LALU</td>
                <td style="border: 0px; width:2px">:</td>
                <td style="border: 0px">Rp. 2.000.000</td>
            </tr>
            <tr>
                <td style="border: 0px; width:500px">Total SP2D DAFTAR PENGUJI INI</td>
                <td style="border: 0px; width:2px">:</td>
                <td style="border: 0px">Rp. 2.000.000</td>
            </tr>
            <tr>
                <td style="border: 0px; width:500px">Total SP2D S/D DAFTAR PENGUJI INI</td>
                <td style="border: 0px; width:2px">:</td>
                <td style="border: 0px">Rp. 2.000.000</td>
            </tr>
        </table>

    </div>
</div>
<br>
<table style="text-align:center; border:0px">
    <tr>
        <td style="border: 0px;">Mengetahui,<br><br><br><br><br><br><br><br><br><br>Nip.
        </td>
        <td style="border: 0px; margin:200px">Mengetahui,<br><br>Kuasa Bendahara Umum Daerah Kota Palu<br><br><br><br><br><br><br><br><br><br><br><br>FADHILA,SE<br><br>Nip.19791113 200804 2 001
        </td>
    </tr>

</table>

</body>
</html>